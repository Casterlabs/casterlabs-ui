...
<script lang="ts">
	...
	onMount(() => {
		...
		element.addEventListener('long-press', handler);

		return () => {
			element.removeEventListener('long-press', handler);
		};
	});
</script>
...
<div style="display: contents;" bind:this={element} data-long-press-delay={DELAY}>
